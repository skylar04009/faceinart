...
def train(hyp, opt, device, tb_writer=None):
    ...
    assert mlc < nc, 'Label class %g exceeds nc=%g in %s. Possible class label are 0-%g' % (mlc, nc, opt.data, nc - 1)
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:
            ...
            if plots:
                #plot_labels(label, names, save_dir, loggers)
                if tb_writer:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 8) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'total', 'label', 'img_size'))
        if rank in [-1, 0]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hyp', type=str, default='data/yolov7.tiny.yaml', help='hyperparameters path')
